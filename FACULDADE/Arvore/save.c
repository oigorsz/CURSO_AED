    ...
    if(raiz == NULL && strcmp(strPai, "fundador") == 0){
        ...
        if(pai != NULL){
            ...
            if(pai->prim == NULL){
                ...
                for(p = p->prim; p->prox != NULL && compData(filho->data, p->prox->data) == 1; p = p->prox);
                //CASO EM QUE DOIS FUNCIONÁRIOS POSSUEM A MESMA DATA, ENTÃO O ADICIONADO POR ULTIMO DEVE ESTAR LOGO APÓS AO MESMO DE SUA DATA
                //CASO GENÉRICO 
                q = p->prox;
                p->prox = filho;
                filho->prox = q;
